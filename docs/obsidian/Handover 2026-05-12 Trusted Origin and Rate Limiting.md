# Handover 2026-05-12 - Trusted Origin and Rate Limiting

Read this first, then [[Thunderstrux Codebase Map]].

Archived from the previous current handover after the public-read-only, trusted-origin, and rate-limiting remediation slices.

## Session Focus

This handover captures the P0 production-readiness hardening slices completed after [[Handover 2026-05-12 Pagination Ownership and Dev Volume]]:

- Removed runtime demo data creation from public event reads.
- Added central trusted-origin protection for custom cookie-authenticated mutation routes.
- Added central Redis-backed fixed-window rate limiting for high-abuse mutation routes.
- Kept Stripe webhook routes exempt from browser-origin and rate-limit guards; they remain protected by Stripe signature verification over raw request bodies.
...
- Trusted-origin tests cover first-party, wrong-origin, and webhook-exemption behavior.
- Rate-limit tests cover allowed/throttled responses, fail-open/fail-closed behavior, signup-before-hash, login-before-NextAuth delegation, checkout side-effect prevention, resend throttling, disabled-local behavior, and log redaction.
...
- `docker compose -f docker-compose.yml -f docker-compose.dev.yml exec app pnpm test:integration` passed with 12 files and 73 tests after trusted-origin and rate-limit coverage.
