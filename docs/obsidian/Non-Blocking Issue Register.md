...
| Stale cleanup operations | Broad stale pending cleanup now depends on an external scheduler running `pnpm stale-orders:process` every 1 minute. | Stale pending orders and expired reservations may remain visible in system/debug views until the worker runs, although checkout remains authoritative before new reservations. | Add deployment scheduler configuration and alert when stale pending or active expired reservation counts exceed threshold. |
