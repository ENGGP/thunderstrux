...
| Dependencies | Direct versions exact-pinned; P2.15 infrastructure in PR #1. 185 integration tests pass locally; all three GitHub jobs passed. Dependency graph/alerts and main required checks active. Baseline: 5 critical/21 high/15 moderate; [[Dependency Advisory Triage 2026-09-18]] records approved separate-remediation direction, owner ENGGP repository maintainer, and committed deadlines. | Bot operation not yet proven; advisories unresolved, not accepted as non-blocking merely by placement here. | Follow [[Dependency Automation]], review/merge PR, activate Renovate, verify controlled and vulnerability PRs and audit notifications. Upgrades require separate review. |
