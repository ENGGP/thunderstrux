...
| Dependencies | Direct versions are exact-pinned; P2.15 validation/audit workflows and broader Renovate config implemented. 185 integration tests pass locally. GitHub dependency graph/alerts active. Baseline audit: 5 critical/21 high/15 moderate; [[Dependency Advisory Triage 2026-09-18]] records proposed decisions, awaiting owners/deadlines. | Bot/required-check operation not yet proven; known advisories unresolved, not accepted as non-blocking merely by placement in this register. | Follow [[Dependency Automation]], approve triage ownership/decisions, activate Renovate/required checks, verify controlled and vulnerability PRs. Upgrades require separate review. |
