...
try {
  const [users, organisations, staff, orders, tickets, reservations, outbox, auditLogs, migrations] = await Promise.all([
    prisma.user.count(),
  ...
  if (migrationCount < 1 || users < 1 || organisations < 1 || staff < 1 || orders < 1 ||
      tickets < 1 || reservations < 1 || outbox < 1 || auditLogs < 1) {
    throw new Error(`Restored database is missing required verification records: ${JSON.stringify({
      migrationCount, users, organisations, staff, orders, tickets, reservations, outbox, auditLogs
    })}`);
  }
